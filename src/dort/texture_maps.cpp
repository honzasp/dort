...
namespace dort {
  ...
  std::shared_ptr<TextureGeom<Vec2>> xy_texture_map(const Transform& tex_to_frame) {
    return make_texture<Vec2, const DiffGeom&>([=](const DiffGeom& geom) {
      Vec3 tex_p = tex_to_frame.apply_inv(geom.p).v;
      return Vec2(tex_p.x, tex_p.y);
    });
  }

  std::shared_ptr<TextureGeom<Vec2>> spherical_texture_map(const Transform& tex_to_frame) {
    return make_texture<Vec2, const DiffGeom&>([=](const DiffGeom& geom) {
      Vec3 tex_p = tex_to_frame.apply_inv(geom.p).v;
  ...
  std::shared_ptr<TextureGeom<Vec2>> cylindrical_texture_map(const Transform& tex_to_frame) {
    return make_texture<Vec2, const DiffGeom&>([=](const DiffGeom& geom) {
      Vec3 tex_p = tex_to_frame.apply_inv(geom.p).v;
      float r = length(tex_p);
      if(r == 0.f) {
  ...
  std::shared_ptr<TextureGeom<Vec3>> xyz_texture_map(const Transform& tex_to_frame) {
    return make_texture<Vec3, const DiffGeom&>([=](const DiffGeom& geom) {
      return tex_to_frame.apply_inv(geom.p).v;
    });
  }
}
