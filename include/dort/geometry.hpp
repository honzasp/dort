...
namespace dort {
  ...
  inline float abs_dot(const Normal& norm, const Vector& vec) {
    return abs(dot(norm.v, vec.v));
  }
  ...
  struct Ray {
    ...
    Ray() = default;
    Ray(const Point& orig, const Vector& dir, float t_min, float t_max = INFINITY):
